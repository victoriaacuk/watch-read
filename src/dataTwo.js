export const dataTwo = [

    {
    id: 1,
    name: "Black Adam",
    date: "October 21, 2022",
    description: "Nearly 5,000 years after he was bestowed with the almighty powers of the Egyptian gods-and imprisoned just as quickly-Black Adam (Johnson) is freed from his earthly tomb, ready to unleash his unique form of justice on the modern world.",
    genre: "Action, Adventure",
    country: "United States",
    image: "https://m.media-amazon.com/images/M/MV5BN2ZmYTlhNmMtODQ0OS00YWNiLWI1MmMtNzkwYWMzOGM4YWQxXkEyXkFqcGdeQXVyMDM2NDM2MQ@@._V1_.jpg"
    },

    {
    id: 2,
    name: "Avatar: The Way of Water",
    date: "December 16, 2022",
    description: "Jake Sully lives with his newfound family formed on the planet of Pandora. Once a familiar threat returns to finish what was previously started, Jake must work with Neytiri and the army of the Na'vi race to protect their planet.",
    genre: "Science fiction, Action",
    country: "United States",
    image: "https://m.media-amazon.com/images/M/MV5BMWFmYmRiYzMtMTQ4YS00NjA5LTliYTgtMmM3OTc4OGY3MTFkXkEyXkFqcGdeQXVyODk4OTc3MTY@._V1_.jpg"
    },

    {
    id: 3,
    name: "Bibliophile Princess",
    date: "October 2022",
    description: "For as long as she could read, Eliana Bernstein—more commonly known as the (Bibliophile Princess)—has preferred the company of books to that of people. However, as a noble, it is her duty to find a suitor. Fortunately for her, Crown Prince Christopher Selkirk Asherald proposes a deal: He will protect her free time, and, in exchange, Eliana will be his fiancée in name only.  The years they spend together are bliss, as Eliana learns to open herself to others. But all good things must come to an end. For Eliana, who loves books more than people, ending their fake relationship should be no big deal. But as Christopher grows close with another girl, Eliana begins to realize that she isn't as indifferent to him as she once thought.",
    genre: "Fantasy, Romance",
    country: "Japan",
    image: "https://img1.ak.crunchyroll.com/i/spire2/389cceacee5b790237974892817dab0d1653975664_main.jpg"
    },

    {
    id: 4,
    name: "I'm the Villainess, So I'm Taming the Final Boss",
    date:"October 2022",
    description: "I’m the Villainess, So I’m Taming the Final Boss follows Aileen d’Autriche, a noblewoman whose engagement is suddenly broken off by her fiancé. She should have been devasted, but instead, that moment is when she realizes she is living inside an otome game. Her fate isn’t looking good: she’s the villain of the story and her life is about to end at the hands of the Demon King! So, Aileen decides to make him fall in love with her in order to save herself.",
    genre: "Isekai (different world, portal fantasy and science fiction)",
    country: "Japan",
    image: "https://img1.ak.crunchyroll.com/i/spire4/03dbc3f9c4378ae11782f3d3462c971e1648007848_main.jpg"
    },

    {
    id: 5,
    name: "You Are My Make Up Artist",
    date:" December 31, 2022",
    description: "Wasita is a famous celebrity who was in the process of planning her marriage to her long-term businessman fiance when she discovered that he has been unfaithful to her. Trying to forget what he did to her, she accidentally ends up having a one-night stand with Gat, her make-up artist, who she thought was gay. To make matters worse, because of that night, she winds up getting pregnant. In order to protect her reputation, Gat offers her a contract marriage. ",
    genre: "Comedy, Romance",
    country: " Thailand",
    image: "https://m.media-amazon.com/images/M/MV5BZjk5ZGE3YmMtYWZhMC00OWYxLWE1ZTEtNTU5M2FmMzc5MDkzXkEyXkFqcGdeQXVyNTkzNjEwMjI@._V1_.jpg"
    },

    {   
    id: 6,
    name: "Little Women",
    date:"September 3, 2022",
    description: "Little Women (Korean: 작은 아씨들; RR: Jag-eun Assideul) is an upcoming South Korean television series directed by Kim Hee-won and starring Kim Go-eun, Nam Ji-hyun, Park Ji-hu and Wi Ha-joon. Based on the 1868 novel of the same name by Louisa May Alcott, it is produced by Studio Dragon for tvN. The series depicts the story of three poor sisters who are close, and how they bravely face Korea's richest family.",
    genre: "Mystery, Family drama",
    country: "South Korea",
    image: "https://m.media-amazon.com/images/M/MV5BMzRlZjkwOWEtMjBhMy00NmM3LWFkMzgtYTFmODIyMWFhNDkyXkEyXkFqcGdeQXVyMTUzOTcyODA5._V1_FMjpg_UX1000_.jpg"
    },

    {   
    id: 7,
    name: "Puss in Boots: The Last Wish",
    date:"December 21, 2022",
    description: "When Puss in Boots learns he has used eight of his nine lives, he sets out on a journey to find the mystical Last Wish and restore the lives he has lost with Kitty Softpaws and his new companion Perro, a therapy dog who was disguised as a cat in Mama Luna’s home, all while also escaping new enemies who are trying to hunt him down due to his reputation after the first film.",
    genre: "Comedy, Animation",
    country: "United States",
    image: "https://upload.wikimedia.org/wikipedia/ru/7/78/Puss_in_Boots_The_Last_Wish_poster.jpg"
    },
    
    {   
    id: 8,
    name: "Mavka: The Forest Song",
    date:"December 29, 2022",
    description: "The boundless Ukrainian forests have long been hiding many secrets and mysteries. Here, among the centuries-old trees, unusual mythical creatures live, who guard their world invisible to the human eye. Mavka  is a charming forest nymph. Her main calling is to protect the Forest and its Heart - the sources of life itself - from any encroachment, including people. Lukasz  is an ordinary rural guy who is fond of music, he puts all his talent into playing the flute. It is thanks to the guy's music that a miracle happens: Mavka and Lukash meet and fall in love with each other. Their union is already almost impossible, but the obstacles become even greater when the mercenary and powerful Kalina appears in the village. Not far from the Forest, she builds a sawmill, which is just a front for her true goal - to get to the Heart of the Forest. Kalina knows that the key to him is Mavka, in whose heart love has already settled and made her vulnerable. To use Mavka, Kalina chooses Lukash, who is in love, and peasants, whose souls are filled with anger and fear, as her weapon. Will Mavka and Lukash be able to protect their feelings? Is Mavka able to save the Forest and her love? Will each of the two lovers make the right choice?",
    genre: "Musical, Fantasy",
    country: "Ukraine",
    image: "https://m.media-amazon.com/images/M/MV5BNDU0Y2RkY2ItNzBiOC00ZTcxLWFhMDgtZmQ0MGFlNTc4NjQyXkEyXkFqcGdeQXVyMzA0OTQzODE@._V1_.jpg"
    },
]
